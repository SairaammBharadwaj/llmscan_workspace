...
def semantic_risk_analysis(
    prompt,
    semantic_model
):

    prompt_embedding=semantic_model.encode(
        prompt,
...
st.set_page_config(
    page_title="LLMScan Research Console",
    layout="wide"
)

st.title(
    "LLMScan — Causal Transformer "
...
if st.button("Analyze"):
    ...
    if prompt.strip()=="":
        ...
        with st.spinner(
            "Running Advanced Causal Analysis..."
        ):
            ...
            try:
                ...
                with col2:
                    ...
                    token_plot=plot_token_importance(
                        token_scores
                    )

                    st.pyplot(token_plot)

                    layer_plot=plot_layer_importance(
                        layer_scores
                    )

                    st.pyplot(layer_plot)

                    heatmap_plot=plot_causal_heatmap(
                        token_scores,
                        layer_scores
                    )
